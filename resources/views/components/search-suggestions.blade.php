<ul id="suggestions"
    class="absolute top-full left-0 w-[360px] bg-white border border-gray-200 rounded-lg shadow-lg mt-1 z-50 list-none max-h-[300px] overflow-y-auto">
            ...
            <a href="http://localhost:8000/patient/{{ $patient->id }}"
                ...
                <div class="flex flex-col">
                    ...
                    @if (isset($patient->date_of_birth))
                        <span
                            class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($patient->date_of_birth)->format('d.m.Y') }}</span>
                    @endif
                    @if (isset($patient->id))
